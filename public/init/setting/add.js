$(function () {
    ...
    // Add setting 
    function add_setting(formData) {
        var formUrl = `${baseUrl}/setting`;
        $.ajax({
            type: "POST",
            url: formUrl,
            data: formData,
            ...
            success: function (response) {
                if (response.hasOwnProperty("errors")) {
                    ...
                    if (response.status === 200) {
                        ...
                        $("#form-add-setting").removeClass("was-invalid");
                        $(".backend-error").remove();
                        setTimeout(() => {
    ...
    $("#submit").on("click", function (e) {
        if ($("#form-add-setting").valid()) { // Triggers validation
            var formData = new FormData($("#form-add-setting")[0]);
            add_setting(formData);
        }
    });
